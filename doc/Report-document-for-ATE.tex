...
HP）には（長府製作所，型番入れる）を用いた。 　Table~\ref{tbl-spec}
に両熱源の主要諸元を示した。Air
HPの一次側熱交換器（室外機）は温室南側の外に設置され，Water
HPの一次側熱交換は温室北西に設置した貯水槽に設置した？？？に接続して行った。
とWater
HPはそれぞれ並列して上記に示したN.RECS-st，N.RECS-hs，N.RECS-ugを並列に接続し，熱源からN.RECSまでの距離は両者でほぼ同一とした。

\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1841}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0597}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.0697}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1642}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1791}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1642}}
  >{\centering\arraybackslash}p{(\linewidth - 12\tabcolsep) * \real{0.1791}}@{}}
...
\bottomrule\noalign{}
\endlastfoot
エコヌクールレオ & 三菱電機 & VEH-712HCC-k & 7.2 & 2.57 & 11.8 & 2.95 \\
冷温水熱源機（地下水熱ヒートポンプ） & 長府製作所 & GSHP-0624X & 6.0 &
1.33 & 6.0 & 1.14 \\
