...
df_day_t <- df_day_t %>%
  filter (Type %in%
            c("Cont_Soil_T_Gerbela",
              "NRECS_ug_whp_Gerbela",
              "NRECS_ug_ahp_Gerbela")) 
#作図するデータ系列を指定
select <- c("Cont_Soil_T_Gerbela",
            "NRECS_ug_whp_Gerbela",
            "NRECS_ug_ahp_Gerbela")
...
# 色テーブル
col_map <- c("Cont_Soil_T_Gerbela" ="darkgreen",
             "NRECS_ug_whp_Gerbela" = "red",
             "NRECS_ug_ahp_Gerbela" ="darkblue")

line_map <- c("Cont_Soil_T_Gerbela" ="solid",
             "NRECS_ug_whp_Gerbela" = "solid",
             "NRECS_ug_ahp_Gerbela" ="solid") 

line_w_map<- c("Cont_Soil_T_Gerbela" = 0.3,
               "NRECS_ug_whp_Gerbela" = 0.3,
               "NRECS_ug_ahp_Gerbela" = 0.3) 

p <- ggplot(df_day_t,
  aes(x = date, y = Mean, color = Type, group = Type, linetype = Type, linewidth = Type)
)+
