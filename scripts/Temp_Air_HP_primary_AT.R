...
#csvからデータ読み込み
data <- read.table("/cloud/project/data/Air_HP_in_out_1st_AT_avg_1_2.csv",sep=",", comment.char="#", header=T) 
...
#データ表示範囲指定
from <- as.POSIXct("2025-08-16 00:00:00", tz = tz_use) 
to   <- as.POSIXct("2025-08-31 23:59:00", tz = tz_use)

# データの整形# データをlong形式に変換
df_long <-
  data |> pivot_longer(
    cols = c("Inlet_1_AT","Inlet_2_AT","Outlet_1_AT","Outlet_2_AT","Inlet","Outlet"),
    names_to = "Type",
    values_to = "Value"
    )
...
ggsave("/cloud/project/figs/Temp_Air_HP_AT.pdf",
       plot = p,
       device=cairo_pdf,
       width = 10,
       height = 10,
