#空気熱源式HPの1次側吸気側と排気側の平均気温と2次側の水温をプロットする

library(dplyr)
library(tidyr)
library(ggplot2)
library(patchwork)
library(cowplot)
library(readr)
library(ggpp)
library(forcats)
library(hms)
library(lubridate)

#ひな形を読み込み
source("source/Theme_Box_line.R")

#タイムゾーンの指定
tz_use <- "Asia/Tokyo"

# RDS から温度データを読み込み
AT_rds <- readRDS("/cloud/project/data/rds/TEMP_Air_HP_in_out_1st_AT_avg_1_2_20250701_1001.rds")
AT <- AT_rds$data #データ部分のみを取り出す

WT_rds <- readRDS("/cloud/project/data/rds/TEMP_AirHP_2nd_WT_20250817_1023_Hampel.rds")
WT <- WT_rds$data #データ部分のみを取り出す
WT_wide <- WT %>%
  select (Time_stamp, Type, Hampel_V) 
WT_wide <- WT_wide %>%
  pivot_wider(names_from = "Type",
              values_from = "Hampel_V")

#データ表示範囲指定
from <- as.POSIXct("2025-08-11 00:00:00", tz = tz_use) 
to   <- as.POSIXct("2025-09-10 23:59:00", tz = tz_use)

data_join <- 
  left_join(AT, WT_wide, by="Time_stamp") %>%
  mutate(Date = as_date(Time_stamp, tz = "Asia/Tokyo")) %>%
  mutate (Hours = as_hms(Time_stamp))%>%
  filter (Hours >= as_hms("0:00:00") & Hours <= as_hms("23:59:00"))

# データの整形# データをlong形式に変換
df_long <-
  data_join %>% pivot_longer(
    cols = -c(Time_stamp,Date, Hours),
    names_to = "Type",
    values_to = "Value"
    )

  
#データ表示範囲にデータを整形
df_long <-  df_long %>%
  filter(Time_stamp >= from, Time_stamp <= to) %>%
  filter(!is.na(Value))


#室外機（1次側の気温）と2次側水温の作図----
select0 <- c("Inlet_avg",
            "Outlet_avg",
            "Snd_in_WT",
            "Snd_out_WT")

select1 <- c("P-in AT",
             "P-out AT",
             "S-in WT",
             "S-out WT")

# 色テーブル
col_map <- c("P-in AT" ="darkblue",
             "P-out AT" = "red",
             "S-in WT" = "orange",
             "S-out WT" = "darkgreen")

df_plot <- df_long %>%
  filter(Type %in% select0, !is.na(Value)) %>%         # ← ここで対象系列だけ残す
  mutate(Type = factor(Type, levels = select0)) %>%         # ← 凡例順も固定
  mutate(Type = fct_recode(Type,
                           "P-in AT" = "Inlet_avg",
                           "P-out AT" = "Outlet_avg",
                           "S-in WT" = "Snd_in_WT",
                           "S-out WT" = "Snd_out_WT"))


p <- ggplot(df_plot,
            aes(x = Time_stamp,
                y = Value,
                color = Type,
                #group = Type
                group = interaction(Date, Type, drop = TRUE)
                )
)+
  geom_line(stat = "identity", position = "identity", size = 0.3) +
  #geom_point(stat = "identity", position = "identity", size = 0.5) +
  scale_color_manual(
    values = col_map,  # 使う色だけ
    breaks = select1,   # 凡例の順序と表示対象を制御
    limits = select1,
    name   = NULL
  ) +
  scale_x_datetime(
    limits = c(from, to),
    breaks = seq(from, to, by = "7 days"),
    minor_breaks = seq(from, to, by = "1 days"),
    date_labels = "%b-%d",
    name = "2025",
  ) +
  
  scale_y_continuous(
    limits = c(-1, 45),
    breaks = seq(0, 45, 5),
    minor_breaks = seq(-1, 40, 1),
    name = "Temperature (°C)"
  ) +
  
  labs(
    title ="Avereage temperature of 2 outdoor units",
    subtitle =NULL,
    caption =NULL,
    #"Changes of air temperature in greehouse during the experiment."
  )+
  
  theme_minimal() +
  Theme_Box_line(base_family = "latin-times") +
  theme(
    legend.position = c(0.305, 0.988),
    #legend.position = "top",
    #legend.position = "right",
    legend.direction = "horizontal",
    #legend.direction = "vertical",
    #legend.margin = margin(1,1,1,1),
    # legend.margin = margin(t = 1, r = 6, b = 1, l = 6, unit = "pt"),
    # legend.background = element_rect(
    #   fill = "white",
    #   colour = "black",
    #   linewidth = 0.1,    # 旧版なら size = 1.2
    #   linetype = "solid"
    #   ),
    panel.grid.major.x = element_line(size = 0.1, color="black"),
    panel.grid.major.y = element_line(size = 0.1, color="black"),
    
  )

ggsave("/cloud/project/figs/Fig_Temp_Air_HP_1st_2nd.pdf",
       plot = p,
       device=cairo_pdf,
       width = 15,
       height = 10,
       units = "cm")
