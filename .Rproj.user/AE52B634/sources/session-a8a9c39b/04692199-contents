...
select1 <- c("Water tank")

# 色テーブル
col_map <- c("Water tank" = "darkgreen")
...
p <- ggplot(df_plot,
  ...
  labs(
    title ="Well tank temperature of water HP",
    subtitle =NULL,
    caption =NULL,
  )+
 ...
 ggsave("/cloud/project/figs/Fig_Temp_Water_HP_tank_WT.pdf",
        plot = p,
        device=cairo_pdf,
        width = 15,
        height = 8,
